let render, scenes, cameras, cameraCtrl;
...
function init() {
  render = new THREE.WebGLRenderer({ canvas: document.getElementById('reveal-effect'), antialias: true, alpha: true });
  render.setSize(window.innerWidth, window.innerHeight);
  document.body.appendChild(render.domElement);

  cameras = new THREE.PerspectiveCamera(conf.perspective, window.innerWidth / window.innerHeight, 0.1, 1000);
  cameras.position.z = conf.cameraZ;
...
function initLights() {
  scenes.add(new THREE.AmbientLight(conf.ambientColor));
...
function getRendererSize() {
  const cam = new THREE.PerspectiveCamera(conf.perspective, cameras.aspect);
